...
function TablePlaceholder({ room, selfId, onCardSelect, onReveal, onResetRound }) {
  ...
  return (
  <div className="bg-white rounded shadow flex flex-col p-3 gap-3">
    
    {/* 1️⃣ Vote results (upar) */}
    {room.revealed && (
      <div className="text-center p-3 bg-gray-50 rounded shadow">
        <p className="text-lg font-semibold">Vote Results</p>
        <div className="flex justify-center gap-4 flex-wrap">
          {Object.entries(room.votes || {}).map(([card, count]) => (
            <div
              key={card}
              className={`p-3 rounded shadow w-20 ${
                room.mostVotedCards?.includes(card)
